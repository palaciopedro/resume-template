...
\textbf{JDBC Integration - Java \& SQL} \\
Developed a Java application using JDBC to establish and manage connections with a relational database. The project implements full CRUD operations using PreparedStatement to securely execute SQL queries and prevent SQL injection. Proper resource management was applied to ensure connections, statements, and result sets were correctly closed, avoiding memory leaks. This project strengthened my understanding of backend fundamentals, database integration, exception handling, and clean code organization.

\textbf{Spreadsheet Automation + API Integration - Python, Pandas, OpenPyXL} \\
Development of a Python application to automate the process of updating Excel spreadsheets, integrating with an API that delivers updated data from the Brazilian Championship Table, eliminating repetitive manual tasks and reducing the risk of errors in data manipulation. The project processes data through a main service (api_service) and directly updates an existing tab, preserving the original file structure without recreating it. It includes file existence validation, modular code organization, and executable (.exe) generation for distribution without the need for an installed Python environment.
